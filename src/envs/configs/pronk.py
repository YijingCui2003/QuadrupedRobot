...
def get_config():
    ...
    gait_config.initial_offset = np.array([0.0, 0.0, 0.0, 0.0], dtype=np.float32) * (
        2 * np.pi
    )
    gait_config.swing_ratio = np.array([0.5, 0.5, 0.5, 0.5], dtype=np.float32)
    config.gait = gait_config

    config.goal_lb = torch.tensor([0.3, 0.0], dtype=torch.float)  # Lin_x, Lin_y, Rot_z
    config.goal_ub = torch.tensor([1.0, 0.0], dtype=torch.float)
    ...
    config.action_lb = np.array(
        [0.5, -0.001, -3, -0.001, -3.0, -0.001, -0.001, -2.5, -0.001]
        + [-0.1, -0.0001, 0.0] * 2
    )
    config.action_ub = np.array(
        [3.999, 0.001, 3, 0.001, 3.0, 0.001, 0.001, 2.5, 0.001] + [0.1, 0.001, 0.2] * 2
    )
