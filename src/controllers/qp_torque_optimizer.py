...
# PDHG
from src.solver.pdhg.qp_solver import QPSolver
...
def compute_orientation_error(
    ...
    desired_quat = quat_from_euler_xyz(
        desired_orientation_rpy[:, 0],
        desired_orientation_rpy[:, 1],
...
def compute_desired_acc(
    ...
    base_quat = quat_from_euler_xyz(
        base_rpy[:, 0], base_rpy[:, 1], torch.zeros_like(base_rpy[:, 0], device=device)
    )
...
def solve_grf(
    ...
    linear_term = torch.bmm(
        torch.bmm(torch.transpose(mass_mat, 1, 2), Q), (g + desired_acc)[:, :, None]
    )[:, :, 0]
    ...
    multiplier = torch.where(
        friction_force < max_friction_force, 1, max_friction_force / friction_force
    )
    if clip_grf:
    ...
    return (
        ...
        solved_acc,
        qp_cost,
        torch.sum(friction_force > max_friction_force + 1, dim=1),
    )
...
def solve_grf_qpth(
    ...
    qf = QPFunction(
        verbose=-1, check_Q_spd=False, eps=1e-3, solver=QPSolvers.PDIPM_BATCHED
    )
    grf = qf(
        quad_term.double(), -linear_term.double(), G.double(), h.double(), e, e
    ).float()
    # print(grf)
    # ans = input("Any Key...")
    # if ans in ["Y", "y"]:
    ...
    return grf, solved_acc, qp_cost, torch.zeros(mass_mat.shape[0], device=device)
...
def solve_grf_pdhg(
    ...
    a = torch.bmm(torch.bmm(torch.transpose(mass_mat, 1, 2), Q), mass_mat)
    b = R  # torch.bmm(torch.bmm(all_foot_jacobian, R), all_foot_jacobian.transpose(1, 2))
    quad_term = a + b
    ...
    # PDHG
    # print(f"P={quad_term.shape}, q={linear_term.shape}, H={G.shape}, b={h.shape}")
    solver = QPSolver(device=device, n=quad_term.shape[1], m=G.shape[1])
    # Solve QP problem:
    # minimize    (1/2)x'Px + q'x
    # subject to  Hx + b >= 0,
    # where x in R^n, b in R^m.
    # P: [num_envs, 12, 12], q: [num_envs, 12]
    # H: [num_envs, 24, 12], b: [num_envs, 24]
    with torch.no_grad():
        X, primal_sols = solver.forward(
            P=quad_term.float(),
            q=-linear_term.float(),
            H=-G.float(),
            b=h.float(),
    ...
    return X, grf, solved_acc


def solve_grf_pdhg_strict(
    mass_mat,
    desired_acc,
    base_rot_mat_t,
    all_foot_jacobian,
    ...
    G = torch.zeros((mass_mat.shape[0], 24, 12), device=device)
    h = torch.zeros((mass_mat.shape[0], 24), device=device)
    base_rot_mat = torch.transpose(base_rot_mat_t, 1, 2)
    for leg_id in range(4):
        G[:, leg_id * 2, leg_id * 3 + 2] = 1
        G[:, leg_id * 2 + 1, leg_id * 3 + 2] = -1
    ...
    J = torch.cat((JT, -JT), dim=1)
    # h_J = torch.zeros((mass_mat.shape[0], 24), device=device)
    h_J = (
        torch.tensor([15.0] * 24, device=device)
        .unsqueeze(0)
        .expand(mass_mat.shape[0], -1)
    )
    H = torch.cat((-G, J), dim=1)
    b = torch.cat((h, h_J), dim=1)
    # PDHG
    solver = QPSolver(device=device, n=quad_term.shape[1], m=H.shape[1])  # n=12, m=48
    # Solve QP problem:
    # minimize    (1/2)x'Px + q'x
    # subject to  Hx + b >= 0,
    # where x in R^n, b in R^m.
...
class QPTorqueOptimizer:
    ...
    def __init__(
        ...
        self.last_X = torch.zeros((robot.num_envs, 24 * 2), device=robot._device)
        self._robot = robot
    ...
    def _solve_joint_torques(self, foot_contact_state, desired_com_ddq):
        ...
        if self._use_full_qp:
            # default: use warm up for pdhg
            self.last_X, grf, solved_acc = solve_grf_pdhg(
                self._mass_mat,
                desired_com_ddq,
            ...
            # clipped least square
            grf, solved_acc, _, _ = solve_grf(
                self._mass_mat,
                desired_com_ddq,
                self._robot.base_rot_mat_t,
                self._Wq,
        ...
        return motor_torques, solved_acc, grf
    ...
    def compute_joint_command(
        ...
        motor_torques, solved_acc, grf = self._solve_joint_torques(
            foot_contact_state, desired_acc_body_frame
        )
        foot_position_local = torch.bmm(
            self._robot.base_rot_mat_t, desired_foot_position.transpose(1, 2)
        ).transpose(1, 2)
